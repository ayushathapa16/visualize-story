        ...
        hitTarget({
          ...
          onActivate: () =>
            openPanel({
              ...
              // `caveat` explains what the figures below it are (and, for the
              // three birds the study couldn't model, why there aren't any).
              // `habitat` is IUCN's own sentence, quoted and attributed.
              body: species
                ? [
                    species.blurb,
                    species.habitat
                      ? `<em>${species.habitat}</em> &mdash; IUCN Red List`
                      : null,
                    species.caveat,
                  ].filter(Boolean)
