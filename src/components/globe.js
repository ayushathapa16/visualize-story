 ...
 *  lower limb - and the lowest stop label hanging off it - stay clear of the
 *  two-voice caption band. Scene 2's camera framing is tuned to match.
 *  (That lowest label used to be "South America"; since the route was corrected
 *  it is "Caribbean", which sits higher, so this framing has margin to spare.) */
const CX = 800;
const CY = 380;
const R = 250;
 ...
 * `ly` nudges a label off its dot.
 *
 * THREE STOPS, AND THAT IS A CEILING RATHER THAN AN ACCIDENT. There were five
 * while the route ran from Brazil, and they were spread over 55° of latitude
 * with room between them. Correcting the range (below) squeezed the whole
 * journey into about 22°, and the same five labels then printed on top of each
 * other over the eastern seaboard - "United States", "Lake Ontario", "Florida"
 * and "Caribbean" inside one small patch of ocean.
 *
 * So the two that were only ever scenery are gone. "Lake Ontario" sat a few
 * pixels from Toronto and needed an `ly: 34` shove to avoid it; Scene 1 has
 * already spent a whole frame on that lake. "United States" was a bend in the
 * line, not a destination. What is left is the two ends of the journey and the
 * coast she crosses to make it - each one a place the narration actually names.
 *
 * If you add a stop, delete one.
 *
 * THE ROUTE STARTED IN SOUTH AMERICA UNTIL 2026-07-27, AND THAT WAS WRONG.
 * Tree Swallows do not winter there. Knight et al. 2019 tracked 133 birds with
 * geolocators from 12 breeding sites: the nonbreeding range is the Gulf of
 * Mexico, Florida, Mexico, Central America and the Caribbean, and birds on the
 * EASTERN flyway - Toronto's end of it - winter in Florida and the Caribbean.
 * The old first stop (lon -60, lat -10) is the Brazilian interior, roughly
 * 4,000 km past anywhere this species goes. See docs/sources.md §G3.
 *
 * The journey is shorter now and the scene survives it: the narration says
 * "thousands of kilometres", which Cuba to Toronto still is. Do not put South
 * America back to make the arc look more impressive.
 */
const STOPS = [
  { lon: -79.0, lat: 21.5, label: 'Caribbean', side: 'right', ly: 22 },
  { lon: -81.5, lat: 27.8, label: 'Florida', side: 'right', ly: 6 },
...
export function globe({ labels = true } = {}) {
  ...
  function drawTo(p) {
    ...
    stops.forEach((l) => {
      // `p > 0` matters: the first stop sits at t=0, so without it the Caribbean
      // pin is on screen from the start - hanging over Africa while the planet
      // is still turning, because the stops are projected at the RESTING
